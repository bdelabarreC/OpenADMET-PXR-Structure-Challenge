...
Because I was new to all of them, a variety of options were used to run each diffusion/cofolding package.  There were several efforts employed to rank the output of the diffusion/cofolding - clustering of results to find the most reproducible pose followed by top 'iptm' measure was best in my hands for creating the highest scoring entries.  Aside - I think the clustering approach may represent reality better than a single crystal structure.  The results are reminiscent of molecular dynamic frames (see https://github.com/bdelabarreC/OpenADMET-PXR-Structure-Challenge/blob/main/run8_x00011_2clusters.mp4) and typically clustered around very reasonable binding options.  The clustering provides useful output in that portions of the molecule that need to be modified can be clearly identified regardless of whether the exactly correct crystallography pose is predicted.
