...
Access to 184 xtl structures has enabled a deeper analysis for each of the efforts.  Showing a plot here of aggregated efforts with RMSD between crystallographic structure and predicted structure computed for all prediction efforts with either Protenix or OpenFold3.  Approximately 1280 predictions for the 184 ligands averaged to 7 predictions per ligand were done before selecting best answer (generally via iptm score) to submit for an entry.  Here showing all 1280 cumulative predictions shows a success rate of almost 90% for RMSD 4 A and under.  In absence of the 'ground truth' answers, selecting the correct pose/structure out of each prediction was the difficult part and this was clearly not always done correctly.  This certainly shows that correct answers can be generated rather easily: no computation run of the 184 ligands took longer than 2 hours.  I estimate total computation time for the data comprising this plot at less than 10 hours.

<img width="1600" height="900" alt="image" src="https://github.com/user-attachments/assets/52f51ecc-7863-4533-8cf2-e787a256e57e" />


No particular molecular property showed strong correlation to either the min, max or the variance of results for the ligands.  A limited analysis suggests that there seems to be some correlation to pseudo-symmetry in the small molecule and reliability of the output.  Pseudo-symmetric molecules (eg x01334) faired poorly in all efforts tried here.
...
<strike>While I have not been able to compare all 20 of the prediction sets sitting on my harddrive against the full 184 structure dataset</strike> After analyzing a diverse subset of my higher scoring prediction entries by direct comparison to the xtallographic data, I also agree that the primary difficulty arose from selecting the correct prediction out of lot of other possibilities.  Multiple seeds/runs produced a range of results and associated metrics simply showed which satisfied a particular but generic protein:ligand interaction model (iPTM).  I tried making several hybrid approaches (shape, pharmacophore, clustering) but none seem to improve on simply picking the best iPTM score.  Given the potential of hitting 90% accuracy across a diverse set of predictions into a large and somewhat flexible pocket I think we should find a way to pluck out those answers that are mostly correct.  Traditional docking methods do not even come close to this metric, so we can say definitely that diffusion based cofolding is a step forward as a protein:ligand binding prediction tool.   

I am curious to see if a co-folding model finetuned on the full 184 structural dataset would be an improvement over current prediction rates.  At a minimum we should have a better train/test set split here such that the test can more directly tell when training has been optimized.  This makes co-folding an ideal approach for a common structure such as PXR faced during drug discovery.  However - most projects I have worked on rarely get past 25 structures before we are either able to nominate a lead or kill the program, so individual datasets will remain small.  Dataset federation may work, but only for those highly competitive targets where most people are unwilling to share truly enabling data.  It remains to be seen if the predictions can be improved (or better selected) when smaller datasets of protein:ligand complexes are available.  Highly dynamic targets (PXR is not this) will also likely prove more challenging. 
